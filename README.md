...
- `sova serve` запускает long polling для Nest Bot API. Бот принимает `/run` или
  нажатие кнопки `Создать обзор` в топике `Chat`.
...
- Дайджесты публикуются только в `Digest`; прогресс, статусы и ошибки уходят в
  `Status`; запросы на подтверждение календарных событий приходят в `Calendar`.
...
После этого в топике `Chat` группы Nest можно отправить `/run` или нажать
кнопку `Создать обзор`. Готовый результат будет отправлен в `Digest`, а не в
`Chat`.
...
перезапусков `serve`, пока активен long polling.
...
| `go run ./cmd/sova serve` | Запускает локальный Nest controller для `/run`, кнопки и daily scheduler. |
...
- Автоматические уведомления, дайджесты и календарные карточки распределяются
  ...
  местом команд и ручного общения.
